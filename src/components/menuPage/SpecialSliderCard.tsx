import Image from 'next/image';
import { HiOutlineShoppingBag } from 'react-icons/hi2';
import { LuEuro } from 'react-icons/lu';

interface ICard {
  name: string;
  srcImage: string;
  description: string;
  price: number;
}

const SpecialSliderCard = ({ name, srcImage, description, price }: ICard) => {
  return (
    <div className=" rounded-lg shadow-lg hover:shadow-xl transition-shadow duration-300 flex flex-col">
      {/* image part */}
      <div className="w-full h-[260px] overflow-hidden rounded-lg md:rounded-t-lg relative">
        <Image
          src={srcImage}
          alt={name}
          fill
          className="object-cover cursor-pointer transform transition-transform duration-500 ease-out hover:scale-110"
        />
      </div>

      {/* content part */}
      <div className="flex flex-col justify-between flex-1 px-8 py-4">
        {/* top content */}
        <div>
          <h3 className="text-[#183136] mb-2 font-extrabold text-xl md:text-2xl">
            {name}
          </h3>
          <p className="text-[#183136] w-full text-left font-light tracking-wider leading-6">
            {description}
          </p>
        </div>

        {/* horizontal line */}
        <div className="my-4 border-b-2 border-dotted border-gray-300" />

        {/* bottom content */}
        <div className="flex justify-between items-center py-3">
          <div className="inline-flex items-baseline">
            <LuEuro
              className="text-[#adb6b7] flex-shrink-0 translate-y-[2px]"
              aria-hidden="true"
              size={19}
              strokeWidth={3}
            />
            <p className="text-[#183136] font-bold text-xl leading-none ml-1">
              {price}
            </p>
          </div>

          <HiOutlineShoppingBag
            size={40}
            className="bg-[#f59d39] rounded-full p-2 transition-all duration-300 hover:bg-[#f0b437] cursor-pointer"
          />
        </div>
      </div>
    </div>
  );
};

export default SpecialSliderCard;
